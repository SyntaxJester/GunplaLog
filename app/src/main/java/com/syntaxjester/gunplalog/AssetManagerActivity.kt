...
class AssetManagerActivity : AppCompatActivity() {
    ...
    private fun categoryPage() {
        ...
        val root = page(); root.addView(header("分类管理", "管理一级分类与子分类", null))
    ...
    private fun categoryMenu(entry: ManagedEntry) {
        ...
        val view = inflater.inflate(R.layout.dialog_category_menu, null) as LinearLayout
        val btnAdd = view.findViewById<AppCompatButton>(R.id.btn_add_child)
        val btnRename = view.findViewById<AppCompatButton>(R.id.btn_rename)
        val btnDelete = view.findViewById<AppCompatButton>(R.id.btn_delete)
        btnAdd.text = "+ 新增子分类"
        btnRename.text = "✏️ 重命名"
        btnDelete.text = "🗑️ 删除一级分类"
        val dialog = AlertDialog.Builder(this).setView(view).show()
        dialog.window?.setBackgroundDrawableResource(android.R.color.transparent)
        btnAdd.setOnClickListener { dialog.dismiss(); childEditor(entry, null) }
        btnRename.setOnClickListener { dialog.dismiss(); editCategory(entry) }
        btnDelete.setOnClickListener { dialog.dismiss()
            if (entry.children.isEmpty()) {
                val all = AssetStore.categories(this); all.removeAll { it.id == entry.id }
                AssetStore.save(this, "categories", all); render()
            } else {
                AlertDialog.Builder(this).setTitle("确认删除")
                    .setMessage("该一级分类下有 ${entry.children.size} 个子分类，删除后无法恢复，确认删除？")
    ...
    private fun childMenu(entry: ManagedEntry, child: String) {
        val inflater = LayoutInflater.from(this)
        val view = inflater.inflate(R.layout.dialog_child_menu, null) as LinearLayout
        val btnRename = view.findViewById<AppCompatButton>(R.id.btn_rename)
        val btnDelete = view.findViewById<AppCompatButton>(R.id.btn_delete)
        btnRename.text = "✏️ 重命名"
        btnDelete.text = "🗑️ 删除子分类"
        val dialog = AlertDialog.Builder(this).setView(view).show()
        dialog.window?.setBackgroundDrawableResource(android.R.color.transparent)
        btnRename.setOnClickListener { dialog.dismiss(); childEditor(entry, child) }
        btnDelete.setOnClickListener { dialog.dismiss()
            entry.children.remove(child); persistCategory(entry); render()
        }
    }
